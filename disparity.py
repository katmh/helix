...
def ncRNADisparities(i, o):
	with open(i) as csv:
		with open(o, 'w') as out:
			for line in csv:
				print(line.split(',')[0])
				out.write(line.split(',')[0] + ',') # write ID
				for value in slide( line.split(',')[1][:-1], 15 ):
					out.write(str(value) + ',')
				out.write('\n')

ncRNADisparities('seq_NONCODE.csv', 'disp_NONCODE.txt')
#generateMeanMax('disp_NONCODE.txt', 'mm_NONCODE.csv')